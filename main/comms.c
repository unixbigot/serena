...
static esp_netif_t *comms_netif = NULL;
...
esp_err_t comms_wifi_init(void)
{
    ...
    comms_netif = esp_netif_create_default_wifi_sta();
    if (comms_netif == NULL) {
...
esp_err_t comms_wifi_connect(char* wifi_ssid, char* wifi_password)
{
...
esp_err_t comms_wifi_disconnect(void)
{
...
esp_err_t comms_wifi_deinit(void)
{
    ...
    ESP_ERROR_CHECK(esp_wifi_clear_default_wifi_driver_and_handlers(comms_netif));
    esp_netif_destroy(comms_netif);
